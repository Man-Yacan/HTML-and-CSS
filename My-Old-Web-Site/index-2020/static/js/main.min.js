...
$window.on('load',function(){$main._show(location.hash.substr(1),true);});setInterval(function countDown(){var oldTime=new Date('2017/07/02 12:00:00'),nowTime=new Date();nowTime=nowTime.getTime();oldTime=oldTime.getTime();var timeDifference=nowTime-oldTime,days=parseInt(timeDifference/(24*60*60*1000)),hours=parseInt(timeDifference/60/60/1000%24),minutes=parseInt(timeDifference/60/1000%60),seconds=parseInt(timeDifference/1000%60);hours=hours<10?'0'+hours:hours;minutes=minutes<10?'0'+minutes:minutes;seconds=seconds<10?'0'+seconds:seconds;document.getElementById('countDownDays').innerHTML=days;document.getElementById('countDownHours').innerHTML=hours;document.getElementById('countDownMinutes').innerHTML=minutes;document.getElementById('countDownSeconds').innerHTML=seconds;},1000);document.oncontextmenu=function(){event.returnValue=false;}
console.log("\n %c 曼亚灿的个人网站 | www.manyacan.com | 个人引导页 ","color:#eee;background:#444;padding:5px 0;");document.body.addEventListener("dblclick",function(){launchFullScreen(document.documentElement);})
function launchFullScreen(element){if(element.requestFullscreen){element.requestFullscreen()}else if(element.mozRequestFullScreen){element.mozRequestFullScreen()}else if(element.webkitRequestFullscreen){element.webkitRequestFullscreen()}else if(element.msRequestFullscreen){element.msRequestFullscreen()}}})(jQuery);